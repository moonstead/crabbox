...
- Bind WebVNC desktop input to viewer sessions when a Linux guest fronts its VNC server with an input gate: the bridge recognises the gate's `CBXGATE 001` greeting, frames traffic and advertises `input_gate`; the coordinator sends each viewer connection a binding derived from its lease and viewer session, relays take and return for the requester's own session through `POST /portal/leases/{lease}/vnc/input` (and `/vnc/embed/input`), which accepts only the coordinator's exact `Origin` and a JSON body, reports `input` in `/vnc/status`, and never lets a viewer's text frame reach a gate; the viewer shows take control or return to agent and reconnects when the gate switches desktops. Guests without a gate are unaffected.
