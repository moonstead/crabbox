...
- Add an unbranded embedded WebVNC viewer for one configured origin: `CRABBOX_WEBVNC_EMBED_ORIGIN` enables `embed: true` viewer tickets, `POST /portal/leases/{lease}/vnc/embed/bootstrap` and `GET /portal/leases/{lease}/vnc/embed`, framed only by that origin with a partitioned session cookie; portal viewer pages keep `frame-ancestors 'none'`.
